...
public interface StockAuditRepository extends JpaRepository<StockAudit, String> {
    ...
    @Query("SELECT s FROM StockAudit s WHERE s.orgId = :orgId AND s.branchId = :branchId ORDER BY s.operationDate DESC")
    Page<StockAudit> findByOrgIdAndBranchIdOrderByOperationDateDesc(
